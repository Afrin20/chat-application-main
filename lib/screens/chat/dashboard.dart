...
import 'package:chat_application_iub_cse464/screens/chat/chat_tabs/discover_page.dart';
import 'package:flutter/material.dart';
...
class DiscoverPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<User> users = [
      User(name: 'John Doe', lastLogin: DateTime.now(), isActive: true),
      User(name: 'Jane Smith', lastLogin: DateTime.now().subtract(Duration(hours: 2)), isActive: false),
      User(name: 'Alice Johnson', lastLogin: DateTime.now().subtract(Duration(days: 1)), isActive: true),
    ];
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            subtitle: Text('Last login: ${user.lastLogin}'), // Show last login time
            trailing: user.isActive ? Icon(Icons.circle, color: Colors.green) : null, // Show "currently active" signal
          );
        },
      ),
    );
...
class User {
  ...
  User({required this.name, required this.lastLogin, required this.isActive});
}
